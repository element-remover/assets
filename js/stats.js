...
const design = {
  ...
  events: function () {
    ...
    function statsCounter(arr, pos) {
      ...
      document.querySelector(`.bl .block:nth-child(${pos + 1}) .bo`).innerText = arr.length > 1 ? [
    ...
    $Arr('.bl .bm').forEach(e => evt(e, "click", evt => {
      let el = evt.target,
        pos = Array.from(el.parentNode.children).indexOf(el);
      $(`.bl .bm.bn`) && cl(`.bl .bm.bn`, "selected", 0);
      cl(el, "selected");
    ...
    $Arr('.bp .by .bz').forEach(e => evt(e, "click", evt => {
      let el = evt.target,
        pos = Array.from(el.parentNode.children).indexOf(el);
      $(`.by .bz.bn`) && cl(`.by .bz.bn`, "selected", 0);
      cl(el, "selected");
    ...
    $Arr('.bp .bq .br').forEach(e => evt(e, "click", evt => {
      let el = evt.target,
        pos = Array.from(el.parentNode.children).indexOf(el);
      $(`.bp .bq .br.bn`) && cl(`.bp .bq .br.bn`, "selected", 0);
      cl(el, "selected");
...
function draw(arr, lines, dur, which) {
  ...
  function font(c, w, s = 16, a = 0) {
    ...
    c.textAlign = !a ? 'center' : a == 1 ? 'bi' : 'left';
  ...
  clean("#graph");
  clean("#points");
  pointerLine($cs("#points"), graph($cs('#bt'), arr, g, lines, dur, which), g)
}